<?php
    ...
    if(isset($_POST['submit'])){
        ...
        $checkquery = "SELECT * FROM `details1` WHERE `Image`='$location_img' && `Name`='$name'";
        $result = mysqli_query($con, $checkquery);
        $num_rows = mysqli_num_rows($result);
        if($num_rows > 0){ ?>
            <script>
                alert('Already Exists, \nPlease Enter another Details...');
                location.replace('details_details1.php');
            </script>
       <?php }
        else{
            $updatequery = "UPDATE `details1` SET `Image`='$location_img', `Name`='$name', `Date`=current_timestamp() WHERE `Sr_No`=$id";
...
<body>
    <?php
        $id = $_GET['update'];
        $selectquery = "SELECT * FROM `details1` WHERE `Sr_No`=$id";
            ...
            while($row = mysqli_fetch_assoc($query)){ ?>
                <section id="container">
                    <form action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>" onsubmit="return validate()" method="post" enctype="multipart/form-data">
                        ...
                        <div class="input">
                            <button type="submit" id="btn1"><a href="details_details1.php">Go Back</a></button>
                        </div>
                    </form>
                </section>
           <?php }
