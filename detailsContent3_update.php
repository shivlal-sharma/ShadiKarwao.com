<?php
    ...
    if(isset($_POST['submit'])){
        ...
        $checkquery = "SELECT * FROM `detailsContent3` WHERE `Content1`='$content1' AND `Content2`='$content2' AND `Name`='$name' AND `Link`='$link'";
       ...
       <?php }
            ...
            $updatequery = "UPDATE `detailsContent3` SET `Content1`='$content1', `Content2`='$content2', `Name`='$name', `Link`='$link', `Date`=current_timestamp() WHERE `Sr_No`=$id";
...
<body>
    <?php
         $id =$_GET['update'];
         $selectquery = "SELECT * FROM `detailsContent3` WHERE `Sr_No`=$id";
