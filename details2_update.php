<?php
    ...
    if(isset($_POST['submit'])){
        ...
        $checkquery = "SELECT * FROM `details2` WHERE `Image`='$location_img' && `Name`='$name'";
       ...
       <?php }
            ...
            $updatequery = "UPDATE `details2` SET `Image`='$location_img', `Name`='$name', `Date`=current_timestamp() WHERE `Sr_No`=$id";
...
<body>
    <?php
        $id = $_GET['update'];
        $selectquery = "SELECT * FROM `details2` WHERE `Sr_No`=$id";
            ...
            while($row = mysqli_fetch_assoc($query)){ ?>
                <section id="container">
                    <form action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>" method="post" enctype="multipart/form-data">
                        ...
                        <div class="input">
                            <button type="submit" id="btn1"><a href="details_details2.php">Go Back</a></button>
                        </div>
                    </form>
                </section>
           <?php }
