<?php
    ...
    if(isset($_POST['submit'])){
        ...
        $checkquery = "SELECT * FROM `detailsContent5` WHERE `Content1`='$content1' AND `Content2`='$content2' AND `Name`='$name' AND `Link`='$link'";
       ...
       <?php }
            ...
            $updatequery = "UPDATE `detailsContent5` SET `Content1`='$content1', `Content2`='$content2', `Name`='$name', `Link`='$link', `Date`=current_timestamp() WHERE `Sr_No`=$id";
...
<body>
    <?php
         $id =$_GET['update'];
         $selectquery = "SELECT * FROM `detailsContent5` WHERE `Sr_No`=$id";
             ...
             while($row = mysqli_fetch_assoc($query)){ ?>
                <section id="container">
                    <form action="<?php echo htmlentities($_SERVER['PHP_SELF']);?>" method="post">
                        ...
                        <div class="input">
                            <button type="submit" id="btn1"><a href="detailsContent5_details.php">Go Back</a></button>
                        </div>         
                    </form>
                </section>
            <?php }
