<?php
    ...
    if(isset($_POST['submit'])){
        ...
        $checkquery = "SELECT * FROM `details4` WHERE `Image`='$location_img' AND `Name`='$name'";
       ...
       <?php }
            ...
            $updatequery = "UPDATE `details4` SET `Image`='$location_img', `Name`='$name', `Date`=current_timestamp() WHERE `Sr_No`=$id";
...
<body>
    <?php
        $id = $_GET['update'];
        $selectquery = "SELECT * FROM `details4` WHERE `Sr_No`=$id";
        $query = mysqli_query($con, $selectquery);
        if($query){
